enum Icons {
  ahmetcetinkaya = "Ahmet Çetinkaya Icon",
  ahmetcetinkayaFilled = "Ahmet Çetinkaya Filled Alternative Icon",
  amazonWebServices = "Amazon Web Services Icon",
  angular = "Angular Icon",
  article = "Article Icon",
  astro = "Astro JS Icon",
  azureDevOps = "Azure DevOps Icon",
  bold = "Bold Icon",
  check = "Check Icon",
  center = "Center Icon",
  close = "Close Icon",
  upArrow = "Up Arrow Icon",
  leftArrow = "Left Arrow Icon",
  rightArrow = "Right Arrow Icon",
  code = "Code Icon",
  copy = "Copy Icon",
  cut = "Cut Icon",
  paste = "Paste Icon",
  edit = "Edit Icon",
  open = "Open Icon",
  eye = "Eye Icon",
  filePlus = "File Plus Icon",
  plus = "Plus Icon",
  computer = "Computer Icon",
  csharp = "C# Icon",
  dart = "Dart Icon",
  donate = "donate",
  doom = "Doom Game icon",
  dotNetCore = ".NET Core Icon",
  downArrow = "Down Arrow Icon",
  drafts = "Drafts Icon",
  file = "File Icon",
  folder = "Folder Icon",
  folderPlus = "Folder Plus Icon",
  envelope = "Mail Icon",
  expressJS = "Express JS Icon",
  flutter = "Flutter Icon",
  formatClear = "Format Clear Icon",
  github = "GitHub Icon",
  git = "Git Icon",
  godotEngine = "Godot Engine Icon",
  handWave = "Hand Wave Icon",
  heading1 = "Heading 1 Icon",
  heading2 = "Heading 2 Icon",
  inbox = "Inbox Icon",
  instagram = "Instagram Icon",
  italic = "Italic Icon",
  itchio = "itch.io Icon",
  java = "Java Icon",
  javascript = "JavaScript Icon",
  kodlamaio = "Kodlama.io Icon",
  laravel = "Laravel Icon",
  link = "Link Icon",
  listSettings = "List Settings Icon",
  linkedin = "LinkedIn Icon",
  properties = "Properties Icon",
  mastodon = "Mastodon Icon",
  maximize = "Maximize Icon",
  meb = "MEB Icon",
  mehmetAkifErsoyUniversity = "Mehmet Akif Ersoy University Icon",
  minimize = "Minimize Icon",
  mngKargo = "MNG Kargo Icon",
  mongodb = "MongoDB Icon",
  mssql = "MS SQL Icon",
  mysql = "MySQL Icon",
  nodeJS = "Node.js Icon",
  nosql = "NoSQL Icon",
  orderedList = "Ordered List Icon",
  php = "PHP Icon",
  postgresql = "PostgreSQL Icon",
  react = "React Icon",
  reboot = "Reboot Icon",
  redis = "Redis Icon",
  refresh = "Refresh Icon",
  send = "send",
  shutDown = "Shut Down Icon",
  solid = "Solid JS Icon",
  spinner = "Spinner Icon",
  sort = "Sort Icon",
  sortAsc = "Sort Ascending Icon",
  sortDesc = "Sort Descending Icon",
  spring = "Spring Icon",
  sql = "SQL Icon",
  terminal = "terminal",
  trash = "Trash Icon",
  typescript = "TypeScript Icon",
  udemy = "Udemy Icon",
  underline = "Underline Icon",
  unity3d = "Unity 3D Icon",
  unorderedList = "Unordered List Icon",
  viewGrid = "View Grid Icon",
  viewList = "View List Icon",
  userSmile = "User Smile Icon",
  userForbidden = "User Forbidden Icon",
  x = "X Icon",
  python = "Python Icon",
  docker = "Docker Icon",
  image = "Image Icon",
  music = "Music Icon",
  video = "Video Icon",
  zoomIn = "Zoom In Icon",
  zoomOut = "Zoom Out Icon",
  rotateClockwise = "Rotate Clockwise Icon",
  rotateAnticlockwise = "Rotate Anticlockwise Icon",
  play = "Play Icon",
  pause = "Pause Icon",
  volumeHigh = "Volume High Icon",
  volumeMute = "Volume Mute Icon",
  fullscreen = "Fullscreen Icon",
  pdf = "PDF Icon",
  spreadsheet = "Spreadsheet Icon",
  archive = "Archive Icon",
}
export default Icons;
